export const handleGoogleSignIn = (setLoader, shop) => {
  ...
  if (window.location.hostname === "localhost") {
    ...
    fetch('/google/auth').then(res => console.log("google autoh done",res))

  } else {
    console.log("Hello 2");
    // window.open(window.location.origin + "/google/auth", "_self");
    fetch('/google/auth', config).then(res => {console.log("google autoh done",res)
  })
...
export function getUser(name, navigate, shop) {
  console.log("shop from google user", shop);
